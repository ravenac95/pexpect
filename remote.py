...
def auto_password(pexpect_session, password, context_shell=None):
    ...
    r = pexpect_session.expect(['(?mi)command not found', '(?mi)No route to host', '(?mi)Name or service not known', '(?mi)Connection refused', '(?i)Host key verification failed', 'continue connecting.*\? ', '(?i)password: |passphrase for key', patt_end])
